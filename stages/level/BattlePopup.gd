...
func moveIn(delay: = false):
	...
	var b1 = get_tree().get_nodes_in_group("battle1")
	var bc = find_node("Battle1Container")
	if b1.size() > 0:
		var abilityName = b1.front().getBattleAbilityName()
		if abilityName == "":
			bc.visible = false
		else:
			bc.get_node("Label").text = abilityName
			bc.visible = true
	else:
		bc.visible = false
