...
func build(data: Array):
	...
	match dome.primaryWeapon:
		"laser":
			...
			$Canvas / BattlePopup.find_node("SpecialContainer").visible = false
			
			$Canvas / BattlePopup.find_node("LeftRightLabel").text = "level.station.battle.navbar.move"
			$Canvas / BattlePopup.find_node("UpLabel").text = "level.station.battle.navbar.shoot"
		"sword":
			$Canvas / BattlePopup.find_node("ActionWeaponMoveUp").visible = false
			$Canvas / BattlePopup.find_node("ActionWeaponMoveDown").visible = false
			$Canvas / BattlePopup.find_node("SpecialContainer").visible = false
			
			$Canvas / BattlePopup.find_node("LeftRightLabel").text = "level.station.battle.navbar.slice"
			$Canvas / BattlePopup.find_node("UpLabel").text = "level.station.battle.navbar.stab"
		"obelisk":
			...
			$Canvas / BattlePopup.find_node("SpecialContainer").visible = true
			
			$Canvas / BattlePopup.find_node("LeftRightLabel").text = "level.station.battle.navbar.move"
			$Canvas / BattlePopup.find_node("SpecialLabel").text = "level.station.battle.navbar.reload"
			$Canvas / BattlePopup.find_node("UpLabel").text = "level.station.battle.navbar.shoot"
			
	$Canvas / TechtreeContainer.visible = true
	$Canvas / GadgetChoicePopup.visible = true
	
	var mode = load("res://content/gamemode/" + loadout.gameMode + "/" + loadout.gameMode.capitalize() + ".tscn").instance()
	add_child(mode)
	...
	Level.keeper = keeper
	Level.dome = dome
	Level.world = loadout.worldScene
	...
	GameWorld.unlockLevelTech(loadout.keeperId)
	GameWorld.unlockLevelTech(loadout.domeId)
	if dome.primaryWeapon:
		GameWorld.unlockLevelTech(dome.primaryWeapon)
	
	if GameWorld.lastPetId != "pet0" and GameWorld.lastPetId != "":
		var pet = load("res://content/pets/" + GameWorld.lastPetId + "/" + Data.startCaptialized(GameWorld.lastPetId) + ".tscn").instance()
		dome.add_child(pet)
	
	$Map.setTileData(loadout.tileData)
	$Map.init()
	$Map.generateCaves()
	
	mode.init()
	
	var tileCount = float(loadout.tileData.get_tile_count())
	...
	if Data.gadgets.has(loadout.primaryGadgetId):
		applyGadget(loadout.primaryGadgetId)
	
	if loadout.gameMode == CONST.MODE_RELICHUNT:
		if GameWorld.gadgetToKeep != "":
			if GameWorld.keptGadgetUsed:
				GameWorld.gadgetToKeep = ""
			else:
				applyGadget(GameWorld.gadgetToKeep)
	...
	if GameWorld.devMode or loadout.savegame:
		startKeeperInput()
	else:
		Level.hud.immediateMoveOut()
		landDome()
	
	SteamGlobal.Steam.connect("overlay_toggled", self, "steam_overlay_toggled")
...
func startGadgetChoiceInput():
	...
	i.popup = find_node("GadgetChoicePopup")
